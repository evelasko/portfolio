...
import { createQuadGeometry, processMarkersForShader, normalizeMarkers } from '../webgl/utils/sphereUtils';
import { createUniform, UNIFORM_PARAMS, normalizeColor } from '../webgl/utils/uniformMapping';
 ...
 */
export const useGlobeEngine = (config: UseGlobeEngineConfig): UseGlobeEngineReturn => {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const engineRef = useRef<WebGLEngine | null>(null);
  const [isReady, setIsReady] = useState(false);

  // Helper function to parse color input
  const parseColor = useCallback((color: string | ColorRGB | undefined, fallback: ColorRGB): [number, number, number] => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const markerColor = parseColor(config.markerColor, { r: 239, g: 68, b: 68 });
      ...
      const globeUniforms: UniformMap = {
        [UNIFORM_PARAMS.resolution]: createUniform('vec2', [config.width, config.height]),
        ...
        [UNIFORM_PARAMS.markers]: createUniform('vec4', new Float32Array(processedMarkers)),
        [UNIFORM_PARAMS.phi]: createUniform('float', config.phi || 0),
        [UNIFORM_PARAMS.theta]: createUniform('float', config.theta || 0),
        [UNIFORM_PARAMS.mapSamples]: createUniform('float', config.mapSamples || 40000),
        [UNIFORM_PARAMS.scale]: createUniform('float', config.scale || 1),
        [UNIFORM_PARAMS.markersCount]: createUniform('float', normalizedMarkers.length),
        [UNIFORM_PARAMS.mapBrightness]: createUniform('float', config.mapBrightness || 0.8),
  ...
  const updateUniforms = useCallback((uniforms: Partial<RenderUniforms>) => {
    ...
    Object.entries(uniforms).forEach(([key, value]) => {
      ...
      if (uniformKey && value !== undefined) {
        if (key === 'baseColor' || key === 'glowColor' || key === 'markerColor') {
          // Type guard for color values
          if (typeof value === 'string' || (value && typeof value === 'object' && 'r' in value && 'g' in value && 'b' in value)) {
            updateData[uniformKey] = {
              type: 'vec3',
              value: parseColor(value as string | ColorRGB, { r: 255, g: 255, b: 255 })
            };
          }
        } else if (key === 'offset') {
          ...
          if (typeof value === 'number') {
            const currentResolution = globeShader.getUniform(UNIFORM_PARAMS.resolution) as [number, number];
            const newResolution: [number, number] = key === 'width' 
  ...
  const setMarkers = useCallback((markers: MarkerLocation[]) => {
    if (!engineRef.current) return;
    ...
    globeShader.updateUniform(UNIFORM_PARAMS.markers, new Float32Array(processedMarkers));
    globeShader.updateUniform(UNIFORM_PARAMS.markersCount, normalizedMarkers.length);
  }, [config.markerSize]);

  // Set rotation function
  const setRotation = useCallback((phi: number, theta: number) => {
