...
export default function LongStripe({ 
  ...
  const distributeImages = (images: string[], featured_images: string[]) => {
    ...
    while (imageIndex < images.length || featuredIndex < featured_images.length) {
      if (isMultiRow && imageIndex < images.length) {
        ...
        if (imageIndex >= images.length && featuredIndex >= featured_images.length) {
          break;
        }
      }
    }
    ...
    if (imageIndex < images.length) {
      ...
      if (remaining.length === 1 && rows.length > 0) {
        ...
        if (lastRow.type === 'multi' && lastRow.images.length < 3) {
          ...
          rows.push({
            type: 'multi',
            images: remaining.length >= 2 ? remaining : [remaining[0], remaining[0]] // Duplicate if only 1 to avoid single-image multi-row
          });
        }
      }
    }
